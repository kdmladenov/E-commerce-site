...
const WishListBtn: React.FC<{ productId: number; isHeartIcon?: boolean }> = ({
  productId,
  isHeartIcon = true
}) => {
  ...
  const [isInWishList, setIsInWishList] = useState(
    JSON.parse(localStorage.getItem('allMyWishList')!)?.some(
      (wish: WishType) => wish.productId === productId
    )
  );
